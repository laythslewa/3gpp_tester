...
void AccessAndMobilitySubscriptionDataRetrievalApi::get_am_data_handler(
  ...
  auto adjacentPlmnsQuery = request.query().get("adjacentPlmns");
  Pistache::Optional<std::vector<PlmnId>> adjacentPlmns;
  if (!adjacentPlmnsQuery.isEmpty()) {
    std::vector<PlmnId> valueQuery_instance;
    if (fromStringValue(adjacentPlmnsQuery.get(), valueQuery_instance)) {
      adjacentPlmns = Pistache::Some(valueQuery_instance);
    }
  }
  ...
  try {
    this->get_am_data(supi, supportedFeatures, plmnId, adjacentPlmns,
