 ...
 * the UE is in suspend state.   - UNKNOWN_REF_ID: Indicates that the PCC rule
 * could not be successfully installed/modified because the referenced
 * identifier to a Policy Decision Data or to a Condition Data is unknown to the
 * SMF.   - INCORRECT_COND_DATA: Indicates that the PCC rule could not be
 * successfully installed/modified because the referenced Condition data are
 * incorrect.   - REF_ID_COLLISION: Indicates that PCC rule could not be
 * successfully installed/modified because the same Policy Decision is
 * referenced by a session rule (e.g. the session rule and the PCC rule refer to
 * the same Usage Monitoring decision data).   - TRAFFIC_STEERING_ERROR:
 * Indicates that enforcement of the steering of traffic to the N6-LAN or 5G-LAN
 * failed; or the dynamic PCC rule could not be successfully installed or
 * modified at the NF service consumer because there are invalid traffic
 * steering policy identifier(s) within the provided Traffic Control Data policy
 * decision to which the PCC rule refers.   - DNAI_STEERING_ERROR: Indicates
 * that the enforcement of the steering of traffic to the indicated DNAI failed;
 * or the dynamic PCC rule could not be successfully installed or modified at
 * the NF service consumer because there is invalid route information for a
 * DNAI(s) (e.g. routing profile id is not configured) within the provided
...
/// UE is in suspend state.   - UNKNOWN_REF_ID: Indicates that the PCC rule
/// could not be successfully installed/modified because the referenced
/// identifier to a Policy Decision Data or to a Condition Data is unknown to
/// the SMF.   - INCORRECT_COND_DATA: Indicates that the PCC rule could not be
/// successfully installed/modified because the referenced Condition data are
/// incorrect.   - REF_ID_COLLISION: Indicates that PCC rule could not be
/// successfully installed/modified because the same Policy Decision is
/// referenced by a session rule (e.g. the session rule and the PCC rule refer
/// to the same Usage Monitoring decision data).   - TRAFFIC_STEERING_ERROR:
/// Indicates that enforcement of the steering of traffic to the N6-LAN or
/// 5G-LAN failed; or the dynamic PCC rule could not be successfully installed
/// or modified at the NF service consumer because there are invalid traffic
/// steering policy identifier(s) within the provided Traffic Control Data
/// policy decision to which the PCC rule refers.   - DNAI_STEERING_ERROR:
/// Indicates that the enforcement of the steering of traffic to the indicated
/// DNAI failed; or the dynamic PCC rule could not be successfully installed or
/// modified at the NF service consumer because there is invalid route
/// information for a DNAI(s) (e.g. routing profile id is not configured) within
